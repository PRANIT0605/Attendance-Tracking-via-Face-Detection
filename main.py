...
while True:
    ...
    if faceCurFrame:
        for encodeFace, faceLoc in zip(encodeCurFrame, faceCurFrame):
            ...
            matches = [bool(match) for match in matches] # converting True/False values of numpy in standard formats
        ...
        if counter!=0:
            ...
            if counter == 1:
                ...
                print(studentInfo)
                # Getting Image Stored in the bucket
                blob = bucket.get_blob(f'Images/{id}.jpeg')
                array = np.frombuffer(blob.download_as_string(), np.uint8)
                imgStudent = cv2.imdecode(array, cv2.COLOR_BGRA2BGR)
                #update data of attendance
                # Check how much time has passed since the last attendance
